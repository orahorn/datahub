...
т.н. двоичный [G-код](https://ru.wikipedia.org/wiki/G-code),
в котором прописаны команды движения механических частей, включение различных
двигателей, остановка и др.
Код для станков [ЧПУ](https://ru.wikipedia.org/wiki/%D0%A7%D0%B8%D1%81%D0%BB%D0%BE%D0%B2%D0%BE%D0%B5_%D0%BF%D1%80%D0%BE%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%BD%D0%BE%D0%B5_%D1%83%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5)
записывается в файл с расширением (суффиксом) cnc.

### Двоичный код внутри вычислительной системы

Такой код, который записывается набором битов.
Такое представление используется для хранения, передачи
и обработки машинным способом в
[комбинационных логических схемах](https://ru.wikipedia.org/wiki/%D0%9A%D0%BE%D0%BC%D0%B1%D0%B8%D0%BD%D0%B0%D1%86%D0%B8%D0%BE%D0%BD%D0%BD%D0%B0%D1%8F_%D0%BB%D0%BE%D0%B3%D0%B8%D0%BA%D0%B0)
(мгновенное преобразование без предыстории),в
[секвенциональных схемах](https://ru.wikipedia.org/wiki/%D0%A1%D0%B5%D0%BA%D0%B2%D0%B5%D0%BD%D1%86%D0%B8%D0%B0%D0%BB%D1%8C%D0%BD%D0%B0%D1%8F_%D0%BB%D0%BE%D0%B3%D0%B8%D0%BA%D0%B0)
(есть предыстория состояния в триггерах).
...
Для того, чтобы передавать битовый (двоичный) код от системы к системы с помощью
электрических сигналов, его подвергают преобразованию, которое так и называется кодирование.
Т.е. в принципе есть понятие представление и
[физическая реализация кодирования](https://ru.wikipedia.org/wiki/%D0%A4%D0%B8%D0%B7%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%BE%D0%B5_%D0%BA%D0%BE%D0%B4%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5).
Это представление реализуется изменением т.н. несущего сигнала - а это ничто иное, как манипуляция
или [модуляция](https://ru.wikipedia.org/wiki/%D0%9C%D0%B0%D0%BD%D0%B8%D0%BF%D1%83%D0%BB%D1%8F%D1%86%D0%B8%D1%8F_(%D0%BC%D0%BE%D0%B4%D1%83%D0%BB%D1%8F%D1%86%D0%B8%D1%8F)) физическим сигналом:

* электрическим напряжением
* частотой колебаний
* сменой фазы сигнала

Самым распространённым исторически считается 
[импульсно-кодовая модуляция](https://ru.wikipedia.org/wiki/%D0%98%D0%BC%D0%BF%D1%83%D0%BB%D1%8C%D1%81%D0%BD%D0%BE-%D0%BA%D0%BE%D0%B4%D0%BE%D0%B2%D0%B0%D1%8F_%D0%BC%D0%BE%D0%B4%D1%83%D0%BB%D1%8F%D1%86%D0%B8%D1%8F),
которая служила цели т.н. оцифровки (а на самом деле дискретизации) аналогово сигнала.
Довольно часто встречается т.н.
[Манчестеское кодирование](https://ru.wikipedia.org/wiki/%D0%9C%D0%B0%D0%BD%D1%87%D0%B5%D1%81%D1%82%D0%B5%D1%80%D1%81%D0%BA%D0%BE%D0%B5_%D0%BA%D0%BE%D0%B4%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5).
...
Эволюция языков программирования ведёт начало от фиксации двоичного кода под запись в файлы.
...
потом этот код описывался символьными мнемониками, а позже появились языки оперирующие 
объектами компьютера более высокого порядка - указателями на память, типизированными переменными,
структурами данных и управления и т.д.
...
* 1958 - LISP - язык обработки списков - первый язык функционального программирования
...
Язык программирования Perl создан лингвистом Larry Wall'ом. Он удобен для программирования в разных парадигмах: алгоритмической, объектно-ориентированнной, структурированной и т.д. Весьма полезны в нём регулярные выражения. Они пришли туда из разных
утилит Unix, вроде grep, но были доработаны и расширены до того, что библиотека совместимых с Perl регулярных выражений (PCRE),
используется в совершенно разных языках программирования и, вообще говоря, требуют отдельного рассмотрения.
...
Для программирования на уровне синтаксиса распределённых и одновременно отказоусточивых систем
для телекоммуникационной отрасли был придуман функциональный язык Erlang и платформа OTP
(Open Telecommunication Platform).
Также поверх Erlang часто используют язык Elixir, который работает на той же виртуальной машине
Erlang.


Практиковаться можно на интерактивном сайте Repl.it:
...
Инсталляция в Debian/Ubuntu'основанных дистрибутивах операционных систем (ОС) ставим пакет:
erlang-base или сам erlang.

Исполнимый файл в консоли: `erl`
...
В ALT Linux:
	...
	apt-get install erlsh erlang-man erlang-doc

Из пакета erlsh установится оболочка скомпонованная с библиотекой readline, со всеми её полезными функциями:

* редактирование
* история


Руководство смотреть можно : 
...
Синтаксис
Для ввода команд запускаем оболочку `erl`.
Она делает приглашение `[цифра]> `
В конце выражения нужно ставить точку (`.`) , чтобы оно вычислялось.

Выход из оболочки erl:

	> q().
	ok

Простейшие типы называются атомами.
Пример числа 10 в шестнадцетиричной системе счисления:

	> 16#A.
	10

Значения типа "атом" стоят без кавычек, начинаются с маленькой буквы.
Пример:

	> aO.
	aO

Кортежи заключаются в фигурные скобки.
Пример:
	> {1,'A',atom_B,13}.
	{1,'A',atom_B,13}

Кортежи нужны для хранения фиксированного набора данных.

Списки в квадратных скобках, предназначены для хранения переменного числа элементов.
Пример:
...
Списки хранят строки с помощью ASCII-кодов.
Пример:

	> [75,69].
	"KE"

Чтобы получить код буквы , перед ней надо поставить знак доллара ($).
Пример:
...
Т.о. строку можно с помощью списка записать так:

	> [$K, $E].
	"KE"

Хотя допускается и простое литеральное значение.
Например:
...
Особенность функционального языка.
Также тип определяется инициализированным значением.

Например, присвоим переменной с именем A значение 10 и выведем его из переменной на экран:
...
Можно присваивать переменным значения через кортежи.
Пример:
...
Есть синтаксис присвоения списка в виде "головы" и "хвоста".
Пример присваивания из списка [0,1,2,3,4,5] головы 0 и хвоста [1,2,3,4,5]:
...
Графический интерфейс пользователя (GUI) делается на wxWidgets
...
Переводится, как Tool Command Language.

Доступен во всех дистрибутивах Unix систем.
Для Windows есть реализация от [ActiveState](https://www.activestate.com/products/tcl/downloads/).


Замечание по выражениям описания синтаксиса (имеется ввиду справочная документация),
которые указывают на необязательность:
TCL использует вместо квадртаных скобок (`[выражение можно опустить]`)
для необязательных выражений знаки вопроса вокруг  (`?необязытельная конструкция?`).
Так принято потому, что выражения в квардратных скобках вызывают функции и подставляют в место вызова значения выводимые функциями на стандартный вывод.
Эту особенность следует учитывать при чтении документации man-руководств операционной системы (ОС).
Раздел 3tcl зарезервирован в справочном руководстве в командной строке.
Т.е. читать про TCL нужно так (пример команды puts):
...
Для установки документации может потребуется выполнить отдельную команду. Например, в Debian-основанных системах:

	apt-get install tcl-doc

Рекомендуемый порядок чтения:

	man Tcl

Интерпретатор языка: `/usr/bin/tclsh`.

Основные пакеты Debian 8:

* `tcl`
* `tcllib` - все библиотеки для работы в TCL
* `libtcl<M>.<N>`

Дополнительные программы:

* wish - интерпретатор при использовании библиотеки [Tk](https://ru.wikipedia.org/wiki/Tk).
* expect - приложение для автоматизации символьных коммуникаций с другими программами.
* tkcon - консоль для работы в Tcl. раскрашивает символами.

Дополнительные пакеты расширения:

* Объектно ориентированные:
* Incr TCL - Имеются Debian пакеты:  `itcl3*`, 
* XOTcl - В Debian пакет xotcl


Разделитель команд языка TCL: `;` (точка с запятой) или новая строка.
Отсюда очевидный вывод: в конце строки точку с запятой можно не ставить.
Комментарий: `#` (решётка). Всё что идёт после `#` является комментарием.
Если есть команда, то желательно комментировать в конце так: `;#` (точка с запятой и потом же решётка).
Отсюда вывод - комментарий - отдельная команда (ничего не делающая) и не может стоять в середине другой команды.

Соединитель строк: \ (обратный слеш). Ставя в конце строки обратный слеш, мы имеем возможность записывать длинные команды
в несколько строк.

Команда присваивания set позволяет устанавливать значения переменным. Пример работы с числом Пи:
```tcl
% set pi 3.1415
3.1415
% puts "Sinus of pi: [expr sin($pi)]"
Sinus of pi: 9.265358966049026e-5
% puts "cosinus of pi [expr cos($pi)]"
cosinus of pi -0.9999999957076562
```

Команда вычислитель: `[выражение]` (внутри квадратных скобок). Вычисляются разные операторы и вызываются функции.
Среди вычисляемых функций:

* `expr` - позволяет считать математические выражения в строке параметре. Например, посчитаем остаток от деления на 13: `puts "[expr 143 % 13]` . Логические выражения возвращают при **истине** 1, а при **лже** - 0. Также работает много фунций, в том числе тригонометрических (см пример выше с косинусом).
	- две звёздочки подряд (`**`) возведение степень . Например: `puts [expr 2 ** 3]` напечатает цифру 8
* `pid` - получить идентификатор процесса. Например: `puts "the pid is [pid]"`

Типы данных: строки (символы в двойных кавычках), числа, массивы. Последние нумеруются от нуля (0).
Спецсимволы в строках - это непечатные и невидимые последовательности, вывод которых производит определённые действия :

* `\r` - carriedge Return - возврат в начало строки курсора.
* `\n` - New line - перевод курсора на новую строчку.
* `\t` - Tabulation - отступ курсору на заданное количество символов (часто 8).
* `\b` - Backspace - возврат обратно на один символ и стирание его.
* также другие и коды символов в восьмеричном и шестнадцетиричном пердставлении.

Атомарные синтаксические конструкции:

* `$имя` - выдаёт значение простой (скалярной) переменной по имени.
* "" - пара кавычек не экранирует разименование (вычисление значений) переменных (через начальный доллар перед их именами).
* `{}` - определяет или набор команд или текстовое выражение, в зависимости от контекста. Например, можно вводить двойные кавычки внутри фигурных скобок, записывать текст на нескольких строчках. Не выполняет вычисления пременных (т.е. выводит знак доллара перед текстом). Не выполняет подстановку команд внутри квадратных скобок (см.далее).
* `[]` - вычисление функции и подстановка её вывода в текущую строку. Допустимо вложение скобок. Пример вычисления числа пи: `puts [expr {asin(1)*2}].`
* `$массив(ключ)` - извлекает из массива значение по ключу. Больше похоже на ассоциативные списки в Perl.


### Переменные

Переменная создаётся командой set.

При этом область действия переменных:

* Вне процедур - глобальная. 
* Внутри процедур - локальная

Внутри процедуры на переменную можно ссылаться, как на глобальную:

```tcl
global переменная
```

Тогда переприсваивания значений с использованием этой переменной в командах set будет проходить с именно глобальнй переменной.
Также к глобальной переменной можно обратиться предварив её имя конструкцией ```::переменная```
.
Отменить переменную можно командой `unset`.

Пример отмены переменной с проверками (оператор `info exist переменная`):
...
* `$переменная(ключ)` - ассоциированный массив
* `$переменная(i,j,...)` - многомерный массив с индексными чилсами i, j, ...
* `$переменная`, `${переменная}` -  подстановка значения переменной

Разыменование возможно и в строках ограниченных двойными кавычками:

```tcl
"$переменная"
```

И не работает внутри строки из фигурных скобок:
...
Т.е. выводится обычная последовательность символов, начиная с символа доллара как есть.

Зарезервированные переменные языка:

* `$argv0` - своё имя скрипта , с которым выполняется текущий процесс. Скалярная строка.
* `$argc - количество параметров (аргументов) переданных сценарию в командной строке.
* `$argv` - массив аргументов, переданный скрипту в командной строке.
* `$env` - массив переменных окружения. например путь к домашнему каталогу можно так получить: `puts $env(HOME)`
* `$tcl_prompt1`  - команда, для переменных первичного приглашения интерпретатора
* `$tcl_prompt2` - вторичное приглашение командной строки. Оно возникает после обратной косой черты в конце строки при разбиении команды на несколько строк. Например, команда `set tcl_prompt2 {puts "> "}` устанавливает символ вторичного приглашения `"> ."`
* `$tcl_version` - версия TCL (не включая уровень исправлений). Например, "8.6"
* `$tcl_patchLevel` - содержит версию (детально до уровня исправления) текущего языка. Например, "8.6.5".
* `$tcl_platform ` - ассоциированный массив с элементами:
	- `$tcl_platform(platform)` - имя платформы (unix, windows, mac...). Например, команда `puts $tcl_platform(platform)` выдаёт `unix`.
	- `$tcl_platform(byteOrder)` - порядок байтов в слове (например, littleEndian)
	- `$tcl_platform(machine)` - аппаратная платформа, на которой сценарий работает (например, x86\_64)
	- `$tcl_platform(osVersion)`  - показывает версию ядра в ОС (например, 3.16.0-10-amd64)
	- `$tcl_platform(os)` - наименование операционной системы (например, Linux)
* `$errorCode` - строка ошибки
* `$errorInfo` - стек ошибок
* `$tcl_interactive` - принимает 1 если команды вводятся с клавиатуры (интерактивно), 0 - при запуске из скриптов (автономно)
* `$tcl_library` - список каталогов со стандартными библиотечными скриптами
* `$tcl_pkgPath` - список каталогов с инсталлированными в них модулями (\*.so разделяемые библиотеки ОС, статические \*.a и \*.tcl скрипты).
* `$tcl_precision` - точность операций с вычислениями с плавающей точкой
* Прочие переменные: `$tcl_traceCompile`, `$tcl_traceExec`


Управляющие конструкции:

* `for {инициализация} {проверка условия} {итерация} {команды}` . В самом начале выполняется блок инициализации, потом проверка условия выполнимости цикла. Если выполнимо, то отрабатывают команды. Замечание: пробелы между парами фигурных скобок обязательны! это же фактически аргументы команды. Пример "вывод положительного числа от 0 до 5": `for {set i 0} {$i <= 5} {incr i} {puts $i}`.
* `foreach` - примерно тоже самое, но в сокращённом конкретном виде.
* `while { условие } { операторы }` - пока условие равно истине - выполняются операторы. Пробел между двумя парами скобок должен быть!
* `break` - прерывание циклов (for, foreach, while).
* `continue` - продолжение цикла (`-//-`)
* `if {условное\_выражение} ?then ?{код1} elseif { условие2 } ?then ?{код2}` - условное выражение - ветвление программы.  Пример проверки количества аргументов в скрипте: `if {$argc < 1} ?then ?{puts "аргументы не заданы"} ?else ?{puts "количество аргументов: $argc"}` . Условное выражение то же самое, что в операторе expr (см. далее).
* !!! команда `case` устарела !!! используйте `switch`:
* `switch "выражение" "образец1" "оператор1" ?образец2 Оператор2? ?...? ?default оператор\_по\_умолчанию?` - операция выбора.
* `proc имя {аргументы} {тело}` - создание процедуры. Внутри тела процедуры нельзя определять новую - вложение не допускается.


Зарезервированные идентификаторы языка:

* `stdout` - стандартный вывод для команды puts
* `stderr` - стандартный вывод ошибки для команды puts
* `stdin` - стандартный ввод для команды gets

Операторы - это комадные выражения:

* `expr выражение`  - вычисление значения математического выражения. expr - используется чаще всего как функция. Поэтому заключается в квадратные скобки. И значение возвращённое оператором подставляется дальше.
* `puts ?-nonewline? ?канал? "строка"` - выводить строку в канал. Если он не указан, то в `stdout`. Ещё вариант `stderr`.
* `gets канал ?переменная?` - вводить (принимать) строкив переменную, если указана. Канал может быть `stdin` -  то есть стандартный ввод. Опция `-nonewline` говорит о том, что после вывода строки, не ставить перенос (`\n` символ -  ASCII десятичный код 10).
* `set <переменная> ?значение?` - если значение не указано, то присвоения не происходит. Вместо этого возвращается текущая переменная. Если переменной не было и значение не указано - то будет ошибка. Т.е. в TCL нет оператора `=`, как в CShell
* `unset переменная` - отмена значения переменной.
* `incr <переменная> ?шаг?` - наращивает значение переменной. По-умолчанию шаг равен единице (1), но может быть даже отрицательным.
* `after <милисекунд>` . Задержать  исполнение на несколько милисекунд. Пример: задержать на полсекунды: after 500


Процедуры и переменные:

Пользовательские команды (процедуры) объявляются командой:

```tcl
proc имя_процедуры {?аргументы?} {команды... }
```

Здесь:

* `имя\_процедуры` - любой уникальный идентификатор.
* `аргументы` - необязательные аргументы, но обязательные фигурные скобки. Т.е. если нет аргументов, то в фигурных скобках - пустое множество. Аргументы друг от друга отделяются пробелами.
* `команды` - обычное перечисление команд с обычными разделителями

Среди команд, важной является:

```tcl
return значение
```

Процедуру можно использовать, как функцию (ни чем не отличается от встроенной) со значениями через пробел:
```tcl
[имя_процедуры значение1 значение2 ...]
```

Для обычного вызова - процедура со значениями через пробел вызывается, как команда:
...
* Языковое расширение [Expect](https://ru.wikipedia.org/wiki/Expect) для автоматизации процессов (в том числе по сети)
* Сетевые инженеры Cisco маршрутизаторы используют TCL, а не Python, Perl и др. сценарных интерпретируемых языках программирования.
* [DejaGNU](https://en.wikipedia.org/wiki/DejaGnu) - мощное приложение проекта GNU Ричаерда Столмана используется для тестриования качества разработок на Си и др. языках программирования. Пишутся на расширении Expect языка программирования TCL сценарии проверки результатов прогона приложений при различных задаваемых входных параметрах. Т.е. эти сценарии запускаются, когда после компиляции программ, когда мы выполняем команды: `make test` или `make check`
* В проектах ПЛИС фирмы Xilinx  и др. электроники.
* Графические приложения на Tk, теснейшем образом интегрируемой с TCL. Например, [tkabber](https://ru.wikipedia.org/wiki/Tkabber)
* Web-приложение [Apache HTTPd TCL](https://tcl.apache.org/) , [Apache Rivet](http://tcl.apache.org/rivet/). На HPE NonStop системах этот модифицированный сервер iTP Secure Server и содержит расширения TCL, на котором написаны конфигурации.
* [TclBlend](https://wiki.tcl-lang.org/page/TclBlend) - интеграция языка TCL с Java платформой. См. также [tclJBlend](https://wiki.tcl-lang.org/page/tclJBlend)
* [Jacl](https://wiki.tcl-lang.org/page/Jacl) - реализация языка TCL целиком на Java
* [tcllib](https://en.wikipedia.org/wiki/Tcllib) - библиотеки к языку TCL на совершенно разные темы. Например, процедуры обработки различнных сетевых протоколов: TCP, UDP и т.д.
* Огромное море привязок и связанных с TCL проектов, которыми укомплектованы различные современные дистрибутивы Linux, BSD и другие Unix-оподобные и не  только операционные системы. Допустим, в основанных на Debian дистрибутивах примерное количество пакетов, так или иначе связыннх с TCL можно оценить набрав команду: `apt-cache search tcl|wc -l`

Приложения на языке TCL/Tk:

* Tkabber - Jabber клиент
* https://github.com/cloudsidekick/tclwinrm - Windows Remote Management interface for Tcl
* gitk - навигатор по репозиторию git.
* tkgate - средство проектирования электрических схем

Интеграция с web-сервером Apache httpd:

Есть возможность делать кодовые вставки на TCL в HTML-страницы на apache сервере httpd .
Пакеты для  Debian 8:

* libapache2-mod-rivet - Server-side Tcl programming system combining ease of use and power.
* libapache2-mod-rivet-doc - Documentation for Rivet, a server-side Tcl programming syste
...
* [TCL  по-русски](https://tcl-ru.blogspot.com/)
