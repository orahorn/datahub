...
пакет модульного тестирования [nunit](https://www.nunit.org/) .

В Ubuntu-18.04 LTS есть отдельные пакеты:

* nunit - сама основа системы
* libnunit-doc - документация и примеры
* nunit-gui - графический интерфейс (фронтенд) для системы тестирования Nunit
* nunit-console - консольный интерфейс для системы NUnit
...
От слова регресс - обратимый. Т.е. это такая методология, когда создаётся набор как автоматических, так и ручных тестов, направленных на проверку функциональности ранее уже написанного и даже протестированного кода на предмет выявления новых ошибок, которые могут появиться как при развитии проекта (появления новых частей кода) либо даже исправлении старых ошибок. Иными словами, если мы исправили код в который внесли новый функционал или устранили старую ошибку - то может в виду видимых или неявных связей возникнуть новая (не выявленная) ошибка.
Таким образом при каждом (или при небольших наборах) исправлении желательно
прогонять полный набор тестов.
Поэтому, предпочтительно писать автотесты на том же или специальном языке.
Подобные автотесты могут запускаться не после внесения изменений (если идёт большой поток), но допустим периодически (особенно ночью, когда нагрузка на системы
разработки, какие-то эталонные стенды меньше).

## DejaGNU

Фреймворк написан на Expect, который в свою очередь написан на  TCL.
Запуск тестов происзодит командой:

	runtest --tool программа_для_тестирования

Ищется каталог `testsuite`, а в нём подкаталог  `программа_для_тестирования` , где запускаются файлы `*.exp`.

Тестирование может проводиться не только в пределах локального хоста, но и по сети или последовательной линни связи 
с использванием программ ssh, rsh, rlogin, telnet, tip и kermit.
